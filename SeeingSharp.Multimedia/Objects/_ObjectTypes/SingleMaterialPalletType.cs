...
{
    public class SingleMaterialPalletType : ObjectType
    {
        ...
        public SingleMaterialPalletType(NamedOrGenericKey palletMaterial)
            : this(palletMaterial, 0.8f, 1.2f, 0.144f, 1f, 0.10f, 0.145f, 0.022f)
        {
        ...
        /// </summary>
        public SingleMaterialPalletType()
            : this(NamedOrGenericKey.Empty)
        {
        ...
        /// </summary>
        public SingleMaterialPalletType(NamedOrGenericKey palletMaterial, float width, float depth, float palletHeight, float contentHeight, float smallFooterWidth, float bigFooterWidth, float boardHeight)
        {
            m_width = width;
            m_depth = depth;
        ...
        public override VertexStructure[] BuildStructure(StructureBuildOptions buildOptions)
        {
            ...
            if (createContent) { result = new VertexStructure[1]; }
            ...
            // Set texture factor on the pallet to zero
            for (int loop = 0; loop < result[0].CountVertices; loop++)
            {
                Vertex actVertex = result[0].Vertices[loop];
                actVertex.TextureFactor = -5f;
                result[0].Vertices[loop] = actVertex;
            }
            ...
            if (createContent)
            {
                result[0].BuildCubeSides16V(new Vector3(0f, m_palletHeight, 0f), new Vector3(m_width, m_contentHeight, m_depth), m_contentColor);
                result[0].BuildCubeTop4V(new Vector3(0f, m_palletHeight, 0f), new Vector3(m_width, m_contentHeight, m_depth), m_contentColor);
                result[0].BuildCubeBottom4V(new Vector3(0f, m_palletHeight, 0f), new Vector3(m_width, m_contentHeight, m_depth), m_contentColor);
            }
            #endregion -----------------------------------------------------------
