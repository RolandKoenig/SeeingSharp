...
namespace FrozenSky.Multimedia.Core
{
    public class ViewRelatedSceneLayerSubset : IDisposable
    {
        ...
        internal void UpdateForView(UpdateState updateState)
        {
            ...
            if(m_anythingUnsubscribed)
            {
                ...
                foreach(var actPassProperties in m_objectsPerPass)
                {
                    ...
                    {
                        throw new FrozenSkyException("Inconsistency: Given unsubscribe count does not mache true count of unsubscriptions!");
                    }
                }
            }
        }
        ...
        internal void Render(RenderState renderState)
        {
            ...
            if (renderState.Device != m_device) { throw new FrozenSkyGraphicsException("Rendering of a ViewRelatedSceneLayoutSubset is called with a wrong device object!"); }
            if (renderState.CurrentResources != m_resources) { throw new FrozenSkyGraphicsException("Rendering of a ViewRelatedSceneLayoutSubset is called with a wrong ResourceDictionary object!"); }
        ...
        {
            if (!m_isSubscribeUnsubscribeAllowed) { throw new FrozenSkyException("Subscription is not allowed currently!"); }
