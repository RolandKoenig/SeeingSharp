...
namespace FrozenSky.Multimedia.Core
{
    public abstract partial class SceneObject : IDisposable, IAnimationHost
    {
        ...
        {
            if (dependency.Scene != this.Scene) { throw new FrozenSkyGraphicsException("Dependency musst have the same scene!"); }
            if (dependency.m_owner != null) { throw new FrozenSkyGraphicsException("Dependency has already an owner!"); }
            if (dependency.DependsOn(this)) { throw new FrozenSkyGraphicsException("Cyclic dependency detected!"); }
            if (m_dependencies.Contains(dependency)) { throw new FrozenSkyGraphicsException("Dependency is already added!"); }
        ...
        {
            if (viewInfo.ViewIndex < 0) { throw new FrozenSkyGraphicsException("Given ViewInformation object is not assoziated to any view!"); }
            if (viewInfo.Scene == null) { throw new FrozenSkyGraphicsException("Given ViewInformation object is not attached to any scene!"); }
            if (viewInfo.Scene != this.Scene) { throw new FrozenSkyGraphicsException("Given ViewInformation object is not attached to this scene!"); }
        ...
        public bool IsStatic
        {
            ...
            set
            {
                if (this.Scene != null) { throw new FrozenSkyException("Unable to change IsStatic state when the object is already assigned to a scene!"); }
                m_isStatic = value;
            }
        }
        ...
        public DetailLevel TargetDetailLevel
        {
            ...
            set
            {
                if (this.Scene != null) { throw new FrozenSkyGraphicsException("Unable to change TargetDetailLevel when object is already added to a scene!"); }
