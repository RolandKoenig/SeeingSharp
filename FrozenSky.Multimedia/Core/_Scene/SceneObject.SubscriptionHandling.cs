...
namespace FrozenSky.Multimedia.Core
{
    public abstract partial class SceneObject
    {
        ...
        // .. if differences are detected, then a FrozenSkyGraphicsException is thrown
        ...
        internal void UpdateSubscription(RenderPassSubscription newSubscriptionInfo, ViewRelatedSceneLayerSubset layerViewSubset)
        {
            ...
            if (entryCount > 1) { throw new FrozenSkyGraphicsException("Inconsistency: Too much subscriptions for SceneObject detected!"); }
        }
        ...
        protected internal void UnsubscribeFromPass(RenderPassInfo passInfo, UpdateState updateState, ViewRelatedSceneLayerSubset layerViewSubset)
        {
            ...
            if (entryCount > 1) { throw new FrozenSkyGraphicsException("Inconsistency: Too much subscriptions for SceneObject detected!"); }
            if (entryCount == 0) { throw new FrozenSkyGraphicsException("Inconsistency: No subscription found on SceneObject!"); }
        }
