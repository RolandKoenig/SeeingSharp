...
namespace FrozenSky.Multimedia.Core
{
    public class RenderState : IDisposable
    {
        ...
        {
            if (m_disposed) { throw new ObjectDisposedException("RenderState"); }
            if (m_sceneStack.Count < 0) { throw new FrozenSkyGraphicsException("There is only one element on the render stack!"); }
        ...
        {
            if (m_disposed) { throw new ObjectDisposedException("RenderState"); }
            if (m_renderSettingsStack.Count < 1) { throw new FrozenSkyGraphicsException("There is only one element on the render stack!"); }
        ...
        /// </summary>
        internal SharpDX.ViewportF Viewport
        {
            get
            {
                if (m_currentRenderSettings == null) { return new SharpDX.Viewport(); }
