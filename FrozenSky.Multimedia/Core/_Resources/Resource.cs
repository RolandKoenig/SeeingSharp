...
namespace FrozenSky.Multimedia.Core
{
    public abstract class Resource : IDisposable
    {
        ...
        {
            if (m_resourceDictionary == null) { throw new FrozenSkyGraphicsException("Unable to load resource: Resource " + m_key.ToString() + " hos no registered ResourceDictionary!"); }
            if (m_device == null) { throw new FrozenSkyGraphicsException("Unable to load resource: Resource " + m_key.ToString() + " hos no registered Device!"); }
        ...
        {
            if (m_resourceDictionary == null) { throw new FrozenSkyGraphicsException("Unable to unload resource: Resource " + m_key.ToString() + " hos no registered ResourceDictionary!"); }
            if (m_device == null) { throw new FrozenSkyGraphicsException("Unable to unload resource: Resource " + m_key.ToString() + " hos no registered Device!"); }
        ...
        public NamedOrGenericKey Key
        {
            ...
            internal set
            {
                if (!m_key.IsEmpty) { throw new FrozenSkyGraphicsException("Unable to change key because there is already a valid key set!"); }
                else { m_key = value; }
            }
        }
