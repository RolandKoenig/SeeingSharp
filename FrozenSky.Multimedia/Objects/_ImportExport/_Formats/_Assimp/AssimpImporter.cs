...
[assembly: AssemblyQueryableType(
    targetType: typeof(FrozenSky.Multimedia.Objects.AssimpImporter),
...
namespace FrozenSky.Multimedia.Objects
{
    ...
    public class AssimpImporter : IModelImporter
    {
        ...
        public ImportedModelContainer ImportModel(ResourceSource sourceFile, ImportOptions importOptions)
        {
            ...
            using(Assimp.AssimpImporter importer = new Assimp.AssimpImporter())
            {
                // Set custom io system
                //  => This is needed to load additional dependencies
                importer.SetIOSystem(new FrozenSkyAssimpIOSystem(sourceFile));
        ...
        private void ImportGeometry(ImportedModelContainer container, Assimp.Scene importedScene, ResourceSource sourceFile)
        {
            for (int actMeshID = 0; actMeshID < importedScene.MeshCount; actMeshID++)
            {
                ...
                Assimp.Color4D[] vertexColors = null;
                if(actMesh.HasVertexColors(0))
                {
                    vertexColors = actMesh.GetVertexColors(0);
                }

                // Get texture coordinates from the model
                Assimp.Vector3D[] textureCoords = null;
                if (actMesh.HasTextureCoords(0))
                {
                    textureCoords = actMesh.GetTextureCoords(0);
                }
                ...
                for (int actVertexID = 0; actVertexID < actVertexCount; actVertexID++)
                {
                    ...
                    newVertex.Position = actMesh.Vertices[actVertexID].ToFrozenSkyVector() * resizeFactor;
                    if (actMesh.HasNormals) { newVertex.Normal = actMesh.Normals[actVertexID].ToFrozenSkyVector(); }
                    if (vertexColors != null) { newVertex.Color = vertexColors[actVertexID].ToFrozenSkyColor(); }
                    if (textureCoords != null) 
                    {
                        ...
                        Vector2 actTexCoord = textureCoords[actVertexID].ToFrozenSkyVector().XY;
                ...
                {
                    uint[] actFaceIndices = actMesh.Faces[actFaceID].Indices;
                    if (actFaceIndices == null) { continue; }

                    // Convert indices to integer format
                    int[] actFaceIndicesInt = new int[actFaceIndices.Length];
                    for(int loop=0; loop<actFaceIndices.Length; loop++)
                    {
                        actFaceIndicesInt[loop] = (int)actFaceIndices[loop];
                    }

                    // Append current face to VertexStructure
                    switch(actFaceIndices.Length)
                    {
                        case 0:
                        case 1:
                        case 2:
        ...
        private void ImportMaterials(ImportedModelContainer container, Assimp.Scene importedScene, ResourceSource sourceFile)
        {
            for(int actMaterialID =0; actMaterialID < importedScene.MaterialCount; actMaterialID++)
            {
                ...
                int difTextureCount = actMaterial.GetTextureCount(Assimp.TextureType.Diffuse);
        ...
        private void ImportObject(ImportedModelContainer container, Assimp.Scene importedScene, Assimp.Node currentNode, SceneObject parentObject)
        {
            ...
            currentPivot.Position = location.ToFrozenSkyVector() * container.ResizeFactor;
            currentPivot.Scaling = scaling.ToFrozenSkyVector();
            currentPivot.RotationQuaternion = rotation.ToFrozenSkyQuaternion();
