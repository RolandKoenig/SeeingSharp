...
namespace FrozenSky.Multimedia.Objects
{
    public partial class VertexStructure
    {
        ...
        public bool Intersects(Ray pickingRay, PickingOptions pickingOptions, out float distance)
        {
            ...
            {
                Vector3 vertex1 = m_vertices[m_indices[loop]].Position;
                Vector3 vertex2 = m_vertices[m_indices[loop + 1]].Position;
                Vector3 vertex3 = m_vertices[m_indices[loop + 2]].Position;
        ...
        public BuiltVerticesRange BuildColumn24V(Vector3 bottomMiddle, float size, float height)
        {
            ...
            return new BuiltVerticesRange(this, (int)startVertex, (int)(m_vertices.Count - startVertex));
        }
        ...
        public BuiltVerticesRange BuildConeFullV(Vector3 bottomMiddle, float radius, float height, int countOfSegments, Color4 color)
        {
            ...
            int bottomVertexIndex = AddVertex(bottomVertex);
            ...
            bool buildSides, bool buildBottom, bool buildTop)
        {
            ...
            //Add bottom and top vertices to the structure
            int bottomVertexIndex = AddVertex(bottomVertex);
            int topVertexIndex = AddVertex(topVertex);
        ...
        public BuiltVerticesRange BuildCube8V(Vector3 start, Vector3 size, Color4 color)
        {
            ...
            int a = this.AddVertex(vertex);
            int b = this.AddVertex(vertex.Copy(new Vector3(dest.X, start.Y, start.Z)));
            int c = this.AddVertex(vertex.Copy(new Vector3(dest.X, start.Y, dest.Z)));
            int d = this.AddVertex(vertex.Copy(new Vector3(start.X, start.Y, dest.Z)));
            int e = this.AddVertex(vertex.Copy(new Vector3(start.X, dest.Y, start.Z)));
            int f = this.AddVertex(vertex.Copy(new Vector3(dest.X, dest.Y, start.Z)));
            int g = this.AddVertex(vertex.Copy(new Vector3(dest.X, dest.Y, dest.Z)));
            int h = this.AddVertex(vertex.Copy(new Vector3(start.X, dest.Y, dest.Z)));
        ...
        {
            BuiltVerticesRange result = new BuiltVerticesRange(this);
        ...
        {
            BuiltVerticesRange result = new BuiltVerticesRange(this);
            result.StartVertex = (int)m_vertices.Count;
            ...
            int a = this.AddVertex(vertex);
            int b = this.AddVertex(vertex.Copy(topB, new Vector2(texX, texY)));
            int c = this.AddVertex(vertex.Copy(topC, new Vector2(0f, texY)));
            int d = this.AddVertex(vertex.Copy(topD, new Vector2(0f, 0f)));
            this.AddTriangle(a, c, b);
            this.AddTriangle(a, d, c);

            // Build Bottom side
            vertex = new Vertex(topA, color, new Vector2(0f, 0f), new Vector3(0f, -1f, 0f));
            a = this.AddVertex(vertex);
            b = this.AddVertex(vertex.Copy(topD, new Vector2(texX, 0f)));
            c = this.AddVertex(vertex.Copy(topC, new Vector2(texX, texY)));
            d = this.AddVertex(vertex.Copy(topB, new Vector2(0f, texY)));
            this.AddTriangle(a, c, b);
            this.AddTriangle(a, d, c);
            ...
            a = this.AddVertex(vertex);
            b = this.AddVertex(vertex.Copy(topB, new Vector2(texX, texY)));
            c = this.AddVertex(vertex.Copy(bottomB, new Vector2(texX, 0f)));
            d = this.AddVertex(vertex.Copy(bottomA, new Vector2(0f, 0f)));
            this.AddTriangle(a, c, b);
            this.AddTriangle(a, d, c);

            // Build Right side
            a = this.AddVertex(vertex.Copy(topB, new Vector3(1f, 0f, 0f), new Vector2(0f, texY)));
            b = this.AddVertex(vertex.Copy(topC, new Vector3(1f, 0f, 0f), new Vector2(texZ, texY)));
            c = this.AddVertex(vertex.Copy(bottomC, new Vector3(1f, 0f, 0f), new Vector2(texZ, 0f)));
            d = this.AddVertex(vertex.Copy(bottomB, new Vector3(1f, 0f, 0f), new Vector2(0f, 0f)));
            this.AddTriangle(a, c, b);
            this.AddTriangle(a, d, c);

            // Build Back side
            a = this.AddVertex(vertex.Copy(topC, new Vector3(0f, 0f, 1f), new Vector2(0f, texY)));
            b = this.AddVertex(vertex.Copy(topD, new Vector3(0f, 0f, 1f), new Vector2(texX, texY)));
            c = this.AddVertex(vertex.Copy(bottomD, new Vector3(0f, 0f, 1f), new Vector2(texX, 0f)));
            d = this.AddVertex(vertex.Copy(bottomC, new Vector3(0f, 0f, 1f), new Vector2(0f, 0f)));
            this.AddTriangle(a, c, b);
            this.AddTriangle(a, d, c);

            // Build Left side
            a = this.AddVertex(vertex.Copy(topD, new Vector3(-1f, 0f, 0f), new Vector2(0f, texY)));
            b = this.AddVertex(vertex.Copy(topA, new Vector3(-1f, 0f, 0f), new Vector2(texZ, texY)));
            c = this.AddVertex(vertex.Copy(bottomA, new Vector3(-1f, 0f, 0f), new Vector2(texZ, 0f)));
            d = this.AddVertex(vertex.Copy(bottomD, new Vector3(-1f, 0f, 0f), new Vector2(0f, 0f)));
            this.AddTriangle(a, c, b);
            this.AddTriangle(a, d, c);
            ...
            result.VertexCount = (int)(m_vertices.Count - result.StartVertex);
            return result;
        }
        ...
        {
            return BuildCube24V(box.Minimum, box.GetSize(), color);
        }
        ...
        public BuiltVerticesRange BuildRect4V(Vector3 pointA, Vector3 pointB, Vector3 pointC, Vector3 pointD, Vector3 normal, TextureCoordinateCalculationAlignment uCoordAlignment, TextureCoordinateCalculationAlignment vCoordAlignment, float coordRepeatUnit)
        {
            ...
            int a = this.AddVertex(vertex);
            int b = this.AddVertex(vertex.Copy(pointB, new Vector2(caluclateU(pointB), calculateV(pointB))));
            int c = this.AddVertex(vertex.Copy(pointC, new Vector2(caluclateU(pointC), calculateV(pointC))));
            int d = this.AddVertex(vertex.Copy(pointD, new Vector2(caluclateU(pointD), calculateV(pointD))));
        ...
        public BuiltVerticesRange BuildCubeSides16V(Vector3 start, Vector3 size, Color4 color)
        {
            ...
            int a = this.AddVertex(vertex);
            int b = this.AddVertex(vertex.Copy(new Vector3(dest.X, start.Y, start.Z), new Vector2(texX, texY)));
            int c = this.AddVertex(vertex.Copy(new Vector3(dest.X, dest.Y, start.Z), new Vector2(texX, 0f)));
            int d = this.AddVertex(vertex.Copy(new Vector3(start.X, dest.Y, start.Z), new Vector2(0f, 0f)));
            this.AddTriangle(a, c, b);
            this.AddTriangle(a, d, c);
        ...
        public BuiltVerticesRange BuildRect4V(Vector3 pointA, Vector3 pointB, Vector3 pointC, Vector3 pointD, Color4 color)
        {
            ...
            int a = this.AddVertex(vertex);
            int b = this.AddVertex(vertex.Copy(pointB, new Vector2(texX, texY)));
            int c = this.AddVertex(vertex.Copy(pointC, new Vector2(texX, 0f)));
            int d = this.AddVertex(vertex.Copy(pointD, new Vector2(0f, 0f)));

            this.AddTriangle(a, c, b);
            this.AddTriangle(a, d, c);

            return new BuiltVerticesRange(this, (int)(m_vertices.Count - 4), 4);
        }
        ...
        public BuiltVerticesRange BuildRect4V(Vector3 pointA, Vector3 pointB, Vector3 pointC, Vector3 pointD, Vector3 normal, Color4 color, Vector2 minTexCoord, Vector2 maxTexCoord)
        {
            ...
            int a = this.AddVertex(vertex);
            int b = this.AddVertex(vertex.Copy(pointB, new Vector2(maxTexCoord.X, maxTexCoord.Y)));
            int c = this.AddVertex(vertex.Copy(pointC, new Vector2(maxTexCoord.X, minTexCoord.Y)));
            int d = this.AddVertex(vertex.Copy(pointD, new Vector2(minTexCoord.X, minTexCoord.Y)));
