...
namespace FrozenSky.Util
{
    public static partial class CommonTools
    {
        ...
        public static FrozenSkyLanguageKey GetLanguageKeyFromFileName(string fileName)
        {
            string[] splittedFileName = fileName.Split(new char[] { '.' }, StringSplitOptions.RemoveEmptyEntries);
            if (splittedFileName.Length < 3) { return FrozenSkyLanguageKey.Default; }

            // normaly the format is <filename>.<language>.<extension>
            string langKey = splittedFileName[splittedFileName.Length - 2]
                .ToLower();
            if (!langKey.StartsWith("lang-")) { return FrozenSkyLanguageKey.Default; }

            // Try to parse the language key
            FrozenSkyLanguageKey result = FrozenSkyLanguageKey.Default;
            Enum.TryParse<FrozenSkyLanguageKey>(langKey.Substring(5).ToUpper(), out result);
            return result;
        }
        ...
        public static string GetLanguageFileCategory(string fileName)
        {
            ...
            if(!fileName.EndsWith(FrozenSkyConstants.FILE_EXTENSION_LANG_WORDS))
            {
                throw new FrozenSkyException("Invalid format of language file (filename: " + fileName + ")!");
            }
            ...
            if (splittedFileName.Length < 2) { throw new FrozenSkyException("Invalid format of language file (filename: " + fileName + ")!"); }
            ...
            {
                throw new FrozenSkyException("No category defined by language file (filename=" + fileName + ")!");
            }
